...
| Repository         | https://github.com/lechihung252/K4-L3-DAY10-H2Q-DataPipeline.git |
...
- **Baseline**: Quality Gate `PASSED (True)`, Freshness SLA `Đạt chuẩn (True, 4.2% stale)`, Retrieval Hit Rate `1.000`, Token F1 `1.000`, Mean Judge Score `5.0`.
- **Corrupted**: Quality Gate `FAILED (False)`, Freshness SLA `Vi phạm cảnh báo (False, 8/22 = 36.4% stale do lùi 365 ngày)`, Retrieval Hit Rate tụt xuống `0.600`, Token F1 tụt xuống `0.779`, Mean Judge Score `4.1`.
- **Repaired**: Quality Gate phục hồi `PASSED (True)`, Freshness SLA `Đạt chuẩn (True, 4.2% stale)`, Retrieval Hit Rate phục hồi `1.000`, Token F1 phục hồi `1.000`, Mean Judge Score `5.0`.
...
1. **Chặn đứng hiện tượng Silent Failure**: Trong hệ thống RAG thực tế, dữ liệu bẩn (null, tóm tắt rỗng, bài báo bị trùng DOI, ngày xuất bản bị lùi 365 ngày) khi nạp vào Vector Database sẽ không gây ra Runtime Error, nhưng làm mô hình LLM bị ảo giác (hallucination) và trả lời sai. Cần một chốt kiểm soát tự động để chặn dữ liệu xấu ngay trước khi index.
...
| ---------------------- | -------: | --------: | -------: | ------------------------- |
| `retrieval_hit_rate` | 1.000 | 0.600 | 1.000 | Giảm 0.400: Do `drop_latest_records` xóa 5 bài mới nhất, trúng 4 câu trong test set nên không retrieve được DOI đúng. |
| `mean_token_f1` | 1.000 | 0.779 | 1.000 | Giảm 0.221: Do 4 câu bị xóa bài gốc dẫn đến câu trả lời lệch từ vựng (2 câu date F1 = 0, 1 câu summary F1 = 0.79). |
| `judge_accuracy` | 1.000 | 0.800 | 1.000 | Gemini chấm 8/10 đúng (2 câu date bị chấm sai do không còn ngữ cảnh đúng). |
| `mean_judge_score` | 5.0 | 4.1 | 5.0 | Điểm đánh giá trung bình từ Gemini giảm từ 5.0 xuống 4.1 ở pha dữ liệu lỗi. |
| Data Quality Gate | PASSED (True) | FAILED (False) | PASSED (True) | Chốt GX 1.x bắt được 2 vi phạm: Unique ID (do `duplicate_rows`) và độ dài summary (do `blank_summary`). |
| Freshness status | Đạt chuẩn (True) | Vi phạm (False) | Đạt chuẩn (True) | Corrupted có 8/22 bài (36.4%) bị stale do lỗi `stale_date` lùi 365 ngày (vượt ngưỡng SLA 25%). |

### Kết luận từ số liệu

1. **Chuỗi nguyên nhân – bằng chứng 1:**
   `[Tiêm 6 lỗi dữ liệu: drop_latest_records, blank_summary, inject_noise, truncate_title, stale_date, duplicate_rows]` $\rightarrow$ `[Quality Gate FAILED (bắt được unique ID và độ dài summary), Freshness SLA vi phạm cảnh báo (8/22 = 36.4% stale do lùi 365 ngày)]` $\rightarrow$ `[Retrieval Hit Rate tụt từ 1.000 xuống 0.600, Mean Token F1 tụt từ 1.000 xuống 0.779, Mean Judge Score giảm từ 5.0 xuống 4.1]`.
2. **Chuỗi nguyên nhân – bằng chứng 2:**
   `[Kích hoạt Idempotent Repair tái tạo từ Raw Snapshot]` $\rightarrow$ `[Quality Gate PASSED 6/6 checks, Freshness phục hồi đạt chuẩn (4.2% stale)]` $\rightarrow$ `[Retrieval Hit Rate lấy lại 1.000, Mean Token F1 lấy lại 1.000, Judge Score trở lại 5.0 hoàn toàn như Baseline]`.

- **Corruption ảnh hưởng rõ nhất và vì sao?**
  Hit Rate và F1 giảm là do `drop_latest_records`: Lỗi này xóa 20% bài mới nhất (5 bài), trong đó có 4 câu `eval_002`, `eval_004`, `eval_007`, `eval_008` hỏi đúng vào các bài bị xóa, khiến hệ thống không thể retrieve được DOI đúng (Hit Rate giảm 1.0 $\rightarrow$ 0.6). `blank_summary` và `truncate_title` không rơi vào câu hỏi nào trong test set nên không ảnh hưởng trực tiếp đến metric. Điều này được chứng minh rõ ràng khi đối chiếu `data/results/corrupted_answers.json` với `data/raw/corruption_log.json` (xem thêm phân tích tại mục 9–10 trong `report/group_report.md`). Đáng chú ý, câu `eval_004` (hỏi authors) dù retrieve sai bài nhưng vẫn được LLM Judge chấm đúng vì trùng tác giả với một bài khác trong corpus — minh chứng kinh điển cho **Silent Failure** mà chỉ metric retrieval mới bóc tách được!
- **Kết quả nào khác với kỳ vọng ban đầu?**
  Ban đầu em dự đoán khi dữ liệu bị lỗi, mã nguồn sẽ báo lỗi đỏ (Crash). Tuy nhiên thực tế hệ thống vẫn chạy trơn tru từ đầu đến cuối mà không có exception nào — đây chính là minh chứng sống động nhất cho hiện tượng **Silent Failure**! Ngoài ra, Quality Gate với 4 Expectations cấu trúc không bắt được lỗi `drop_latest_records` (vì 22 dòng vẫn nằm trong khoảng 5–5000), mà phải kết hợp quan sát Freshness SLA (36.4% stale) và Retrieval Hit Rate mới phát hiện toàn diện.
